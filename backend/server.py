...
async def defaulters_report(class_name: Optional[str] = None, min_due: float = 0, user=Depends(require("principal"))):
    ...
    for s in students:
        ...
        due = due_by_student.get(s["id"], 0)
        if class_name and cls != class_name:
            continue
        if min_due and due < float(min_due):
            continue
        groups.setdefault(cls, []).append((s, due))
    if not groups:
        raise HTTPException(404, "No defaulters match the selected filters.")
    inst = await db.institutes.find_one({"id": user["institute_id"]})
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.units import cm
    from reportlab.lib import colors
    from reportlab.pdfgen import canvas
    buf = io.BytesIO(); c = canvas.Canvas(buf, pagesize=A4); w, h = A4
    rpt_title = "Fee Defaulters Report"
    if class_name:
        rpt_title += f" — {class_name}"
    if min_due:
        rpt_title += f" (Due >= Rs.{int(float(min_due))})"
    draw_letterhead(c, inst, w, h, rpt_title)
    y = h - 4.4 * cm
    grand = 0
...
async def fee_receipt(fee_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if sb:
            from reportlab.lib.utils import ImageReader
            c.drawImage(ImageReader(io.BytesIO(sb)), w / 2 - 1.5 * cm, 1.95 * cm, width=3 * cm, height=3 * cm, mask='auto', preserveAspectRatio=True)
            c.setFillColor(colors.HexColor("#64748B")); c.setFont("Helvetica", 7.5)
            c.drawCentredString(w / 2, 1.75 * cm, "Authorised Signatory / Official Seal")
    except Exception as e:
        logger.warning(f"receipt seal failed: {e}")
    c.setFillColor(colors.HexColor("#64748B")); c.setFont("Helvetica-Oblique", 8)
