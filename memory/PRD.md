...
## Consent block REMOVED (2026-06, forked session, part 19)
- Removed the mandatory Verifiable Parental Consent gate: `create_student` no longer 400s when consent is absent (the block was actually breaking student creation since the form no longer sent it). Registration succeeds immediately.
- Simplified UI: a **pre-checked** checkbox "Parental consent verified by the institute." on the student registration form (emerald style, not mandatory, no OTP/link verification). Save button is not gated by it.
- No login is blocked by consent for any role (login only gates on institute `status`, unchanged).
- **Data tagging kept** for privacy standards: every student still stored with `is_minor_data: true`, `data_classification: restricted`, `pii_category: minor_sensitive`, and a `parental_consent` record ("Parental consent verified by the institute.") — none of which trigger UI blocks.
- VERIFIED (curl): POST /api/students with no consent field → 200; DB retains is_minor_data + tags. Frontend compiles clean.
...
## ID Card fields + Unified Student/Parent Portal (2026-06, forked session, part 20) — DONE
- **P0 fix**: removed orphaned/duplicated JSX tail in `Students.jsx` (Unterminated string constant) that was breaking the whole webpack build. Frontend compiles clean again.
- **Student form fields**: added Date of Birth (student-dob), Blood Group (student-blood-group), Home Address (student-address) inputs to Add/Edit Student modal. Wired end-to-end: `StudentIn` model has dob/blood_group/address; create uses model_dump (saves them); `update_student` PUT `allowed` set now includes dob/blood_group/address/email/roll_no so edits persist. Removed a dead DUPLICATE `PUT /api/students/{sid}` route (the dict-based one at ~1100 is authoritative). IDCard.jsx renders DOB/Blood Group/Father's Name/Emergency/Address + school branding.
- **Unified User Portal** (`Dashboard.jsx` StudentDash rewrite, data-testid=student-portal): serves student & parent from one Student-ID login. Sections: profile hero (name, class·section, student ID, roll no, View ID Card + Report Card buttons), KPI stats (attendance/avg/fees due/pending homework), Today's Classes (today_timetable), Pending Homework (unsubmitted), Latest Marks, Fee Status (total overdue + Pay Now + downloadable recent receipts via `/fees/{id}/receipt`), monthly Attendance Calendar (color-coded present/absent, today ringed), AnnouncementsPanel, and AI Progress Report (StudentInsights: growth score, AI summary, subject bars, 7-day plan).
- **Backend**: `GET /api/dashboard/student` extended to also return profile{name,student_id,class_name,section,batch_name,roll_no,photo_url}, today_timetable, recent_marks, attendance_calendar (current month). pending_fees now = amount-paid across non-paid fees.
- **Simplified student sidebar** (Layout.jsx): Dashboard, My Attendance, Online Tests, Homework, Fees & Receipts, Raise Complaint, Digital ID Card, Change Password (removed Timetable/My Results/AI page/Announcements/Gallery — timetable+marks+announcements now on dashboard).
- **Floating Study Buddy widget** (`StudyBuddyWidget.jsx`): FAB bottom-right on all student pages (data-testid=study-buddy-fab / study-buddy-panel / study-buddy-input / study-buddy-send). Reuses `POST /api/student/ai-assistant` — 24/7 academic doubt-solver, addresses student by name, knows their class, declines off-topic. Old full-page `/app/assistant` route still exists (removed from sidebar).
- VERIFIED via screenshots (minted student token for active-institute student Saanvi Singh / DP20260001, institute 42c4f12d): portal hero + KPIs + Today's Classes + Pending Homework + Latest Marks + Fee Status + receipts render; attendance calendar color-codes days; Study Buddy replies "12 x 8 = 96..." addressed to Saanvi (academic, personalized). Fixed a double-₹ (money() already prefixes ₹).

## ⚠️ Testing note
- Login is 2FA and student IDs (DP2026XXXX) are DUPLICATED across institutes (one inactive DPCS + one active DPCS + others), so `identifier`-based login is ambiguous for automation. For verifying student flows, mint a JWT directly with make_token/JWT_SECRET (HS256) for a student in an ACTIVE institute (e.g. 42c4f12d Delhi Public Convent School) and inject into localStorage key `edusync_token`.

## QR Sticker Batch PDFs + Parent Read-Only Portal (2026-06, forked session, part 21) — DONE
- **QR Sticker PDFs** (reportlab + qrcode, A4, 24/page 4x6 grid, cut-guide borders): `GET /api/print/qr-stickers/students/{batch_id}` (principal/teacher) and `GET /api/print/qr-stickers/faculty` (principal). Each sticker = large scannable QR + name + "Roll:"/"Staff ID:". QR encodes student_id / faculty_id (what the attendance scanner reads). Purpose: print & paste on existing ID cards for instant digital attendance. UI: "Generate QR Stickers" button on BulkIDCards.jsx (per batch) and FacultyIDCards.jsx (all staff) — downloads PDF via auth fetch (data-testid qr-stickers-btn / qr-stickers-faculty-btn).
- **Scanner extended for faculty**: `POST /api/attendance/scan` now falls back to faculty lookup by faculty_id → marks teacher_attendance present (idempotent), returns type:"faculty". Student path returns type:"student". This makes faculty stickers actually enable staff digital attendance.
- **Parent read-only portal**: new `POST /api/auth/parent-login {student_id, parent_email}` → matches student_id + parent_email (prefers ACTIVE institute, handles duplicate student_ids across institutes) → emails OTP → verify-otp issues a role="parent" JWT bound to the child student.id. get_current_user + verify_otp map parent→students collection; parent's must_change_password forced False (no password to change). Parent reuses the unified StudentDash (read-only): all read endpoints (/dashboard/student, /fees, /homework, /timetable, /students/{id}, /insights, /report, /fees/{id}/receipt) now accept role parent scoped to the child; write endpoints (submit homework, attempt quiz, AI assistant) stay require("student") → 403 for parent. Parent CAN pay fees (razorpay) + download receipts. create_complaint accepts parent.
- **Message Class Teacher**: `/dashboard/student` now returns class_teacher{name,phone} (from the child's batch teacher). StudentDash renders a "Message Class Teacher" card (data-testid message-teacher-card) with a WhatsApp click-to-chat button (wa.me, auto 91 prefix, data-testid message-teacher-whatsapp) + "Message via app" (→ Complaints, data-testid message-teacher-app).
- **Login UI**: Landing.jsx adds a "Are you a parent? Sign in here" toggle (data-testid switch-to-parent) → parent form (Student ID + Parent Email, data-testid parent-student-id / parent-email / parent-login-submit) → shared OTP step. Layout has a NAV.parent sidebar (Dashboard, Fees & Receipts, Message Teacher, Digital ID Card) + "Parent" role label; Study Buddy FAB stays student-only.
- VERIFIED: QR student+faculty PDFs (200 application/pdf, rendered to PNG — clean scannable grid). Parent OTP flow end-to-end (parent-login→otp_required→verify-otp→role parent), parent dashboard shows child + class_teacher, /fees child-scoped, cross-student /report → 403, auth/me role=parent mcp=False. Screenshot of parent portal (Parent Portal label, read-only sidebar, Message Teacher card w/ WhatsApp) confirmed. Frontend compiles clean.

## Comprehensive Admission Form + Sticker Sheet Options (2026-06, forked session, part 22) — DONE
- **Admission form upgrade** (Students.jsx): now captures Photo, Full Name, Age, Gender, Class & Section (batch dropdown), Roll No, Father's Name, Mother's Name (new), Primary Contact (parent_phone), Emergency Contact (new), Parent/Student Email, DOB, Blood Group, Home Address, and multi Document uploads (Birth Certificate etc. via /upload → stored in student.documents[{name,url}]).
- **Mandatory fields** (red * + client validation + disabled submit): Full Name, Class & Section, Roll No, DOB, Blood Group, Emergency Contact. save() blocks with a toast listing any missing.
- **Backend**: StudentIn gained mother_name, emergency_contact, documents; update_student allowed-set expanded; create_student no longer overwrites documents:[] (form docs persist). Verified: POST /students with full payload persists mother_name/emergency_contact/documents.
- **Instant ID, no blank dashes**: IDCard.jsx now prefers emergency_contact (→ parent_phone fallback), and FILTERS OUT any empty detail row instead of printing "—". Class/Sec only shows when present. Verified on rendered cards: fully-populated student shows all rows; sparse students show only rows with data (no dashes).
- **Sticker Sheet Options**: `_qr_sticker_pdf(items, per_page, preset)` supports per_page 12 (3x4) / 24 (4x6) / 30 (5x6); preset "standard" (cut-guide borders, 8mm margins) or "avery" (no borders, A4 label margins 4.7mm/12.7mm). Endpoints accept ?per_page=&preset=. UI: two Selects (data-testid stickers-perpage / stickers-preset [+ -faculty]) on BulkIDCards.jsx and FacultyIDCards.jsx next to Generate QR Stickers. Verified via rendered PDFs (12/standard and 30/avery layouts correct).

## Admission Form Validation + Live ID Preview (2026-06, forked session, part 23) — DONE
- **Live ID-card preview** (Students.jsx): the Register/Edit dialog is now a 2-column layout (max-w-3xl) — form on the left, a real-time IDCard preview on the right (data-testid id-preview, hidden on mobile). Preview builds a student object from the live form state (name, photo, roll, class/section derived from the selected batch, father's name, DOB, blood group, emergency) with institute branding; student_id shows "AUTO ON SAVE" for new admissions.
- **Format validation**: phoneOk (10–15 digits, optional + prefix) and emailOk (standard regex) helpers. Inline red errors under Primary/Emergency Contact (data-testid err-parent-phone / err-emergency) and Parent/Student Email (err-parent-email / err-email). save() blocks on invalid phone/email (toast) in addition to the mandatory-field check, and the Save button is disabled while any format error (hasErrs) or missing mandatory field exists.
- NOTE: this task also re-applied the part-22 mandatory-field validation in save() (Name, Class & Section, Roll No, DOB, Blood Group, Emergency Contact) — it had silently not landed earlier; now present and verified.
- VERIFIED via screenshot: live preview updates as fields are typed (Aarav Mehta / Roll 27 / Mr. Mehta shown on card); invalid phone → "Enter 10–15 digits", invalid email → "Invalid email address", Save disabled. Frontend compiles clean.
...
## Certificate Generator + Dynamic Class Fee Structure (2026-06, forked session, part 26) — DONE
- **Certificate Generator** (/app/certificates, principal; page Certificates.jsx): 6 types (Achievement, Participation, Sports, Bonafide, Character, Transfer). Auto-fills student name, father's name, class, roll no, student ID, session into type-specific boilerplate. POST /api/certificates creates a record (cert_no CERT-YYYY-####, unique verify_code). GET /api/certificates/{id}/pdf → branded A4 PDF (letterhead + watermark + gold border + body + signature line + QR encoding {base}/verify-cert/{code}). PUBLIC GET /api/certificates/verify/{code} (no auth) → validity + details; public page VerifyCertificate.jsx at /verify-cert/:code shows a green Verified / red Not Found card. Copy-verify-link button on each row.
- **Dynamic Class Fee Structure** (/app/class-fees, principal; page ClassFees.jsx): per-grade (Nursery..Class 12) component list (Tuition/Transport/Lab/Exam/Sports etc). Collection fee_structures {grade, components:[{name,amount}], total, frequency}. Endpoints: GET /api/fee-structures, POST (upsert by grade), DELETE /api/fee-structures/{id}.
- **Auto-allocation on admission**: create_student now calls allocate_class_fees() — matches the student's batch (grade/class_name/name, normalized, prefix-safe so Class 1 ≠ Class 12) to a fee_structure and auto-creates a pending fee for the current month with the structure's components as itemized line items. Verified: admitting to Class 9-A with a Class 9 structure (Tuition 3000 + Transport 800) auto-created a ₹3,800 pending fee.
- **Fee Management** (existing, confirmed): shows total dues/paid/balance, full (mark-paid / Razorpay) + partial (pay-partial) payments, and itemized branded receipts (fee_receipt PDF lists each component). Nav: added "Class Fee Setup" + "Certificates" for principal.
- VERIFIED via curl (structure upsert, auto-alloc ₹3800, cert create+public verify valid+PDF 200) and screenshots of both new pages. Backend + frontend compile clean. Test data cleaned up.

## ID Card Polish + Certificate Templates + Fee Auto-Schedule (2026-06, part 27) — DONE
- **ID Card** (IDCard.jsx): label "Emergency"→"Contact Number"; removed institute code (EDU2) and the "Scan for attendance/verify" line from footer; QR now bottom-left with the full Address (multi-line, 3-line clamp) to its right — no overlap with QR or contact number. Verified via screenshot.
- **Certificate Templates**: CertificateIn + BulkCertIn gained signatory_name & signatory_designation. Cert PDF renders the signatory name + designation on the signature line and stamps the institute logo as an official seal above it. New POST /api/certificates/bulk issues a certificate to every student in a batch (verified: 14 for Class 9-A). Certificates.jsx form adds Signatory Name/Designation inputs + a "Bulk issue to a class" batch picker & button.
- **Fee Auto-Schedule**: .emergent/crons.yml adds `monthly-class-fees` (cron "0 2 1 * *") → POST /api/cron/generate-monthly-fees (Bearer WEBHOOK_CRON_SECRET, hmac.compare_digest, backgrounds work via asyncio.create_task, acks 2xx). _generate_monthly_fees() creates a pending itemized fee for the current month for every student whose class matches a fee_structure (idempotent per student+month). Verified: 401 no-auth, 200 with secret, Saanvi got ₹3500 (Tuition+Lab) for the current month; test data cleaned up. (Note: existing fee-reminders cron kept; 2 crons total.)

## Fee Defaulter Report + Certificate Seal Upload (2026-06, part 28) — DONE
- **Fee Defaulter Report**: GET /api/fees/defaulters-report (principal) → branded A4 PDF grouping every student with outstanding balance (amount-paid across non-paid fees) BY CLASS, with roll/name/contact/due, per-class subtotals and a grand Total Outstanding. Button "Defaulter Report" added to Fee Management header (Fees.jsx, downloadPdf). Verified: PDF 200, ₹64,500 total across classes.
- **Certificate Seal Upload**: added _seal_bytes(inst) (reads institute.seal_path from object storage, falls back to logo); certificate PDF now stamps the custom seal instead of the logo. Institute PATCH allowed-set gained seal_url/seal_path. Certificates.jsx: "Official Seal / Signature" uploader (data-testid cert-seal-upload) → /upload then PUT /institute {seal_url,seal_path} → refreshInstitute; shows current seal preview. Verified: uploader renders, compiles clean.
- NOTE: recurring duplicate-tail corruption hit Certificates.jsx again during edits; fixed (removed orphaned block) and re-added the seal UI. All verified.

## Premium Certificate Template (2026-06, part 29) — DONE
- Master template image saved to /app/backend/assets/cert_template.png (from user URL) and used as a full-page LANDSCAPE (A4 landscape) background for all certificate types.
- certificate_pdf rewritten: dynamic title "CERTIFICATE / OF <TYPE>", "proudly presented to", student Full Name in elegant script style (Helvetica-BoldOblique, gold underline), auto-filled body (name/class/roll/reason/session), signature block (institute seal/logo + signatory name & designation), Date + Institute name on the right, and a small QR code in the top-left corner for online verification. Semi-opaque white "cards" (0.95 alpha, rounded) mask the template's baked placeholder text behind each overlay for a clean glassy look.
- Output: high-quality printable landscape PDF (verified 841x595, 200 OK; rendered PNG confirms all overlays clean). No frontend change needed — existing Download/PDF actions open the new landscape certificate.

## Automated Faculty Salary Management (2026-08-15, part 30) — DONE
- **Backend (was already built, now verified)**: `_gen_salary` computes Basic (~45% CTC), HRA (40/50% metro), Special allowance, EPF/PT/TDS, and LWP = gross ÷ days-in-month × (rejected-leave days + attendance-based absent working days Mon–Sat). Endpoints: `POST /api/salaries` (generate, 409 on duplicate teacher+month), `PATCH /api/salaries/{id}` (manual adjust: extra_allowance/extra_deductions/note, recomputes net; blocked on paid), `PUT /api/salaries/{id}/pay` (issues slip_no), `GET /api/salaries/{id}/slip` (branded watermarked PDF), `POST /api/salaries/{id}/email-slip`.
- **Monthly cron**: `.emergent/crons.yml` → `monthly-faculty-salaries` (`0 3 1 * *`) → `POST /api/cron/generate-monthly-salaries` (Bearer WEBHOOK_CRON_SECRET, backgrounds work, idempotent per teacher+month).
- **Code cleanup**: removed the duplicate `GET /salaries/{id}/slip` route (there were two definitions); kept the watermarked adjustment-aware slip used by both HTTP download and email.
- **Frontend (NEW, this session)**: `Salary.jsx` now has a per-row **Adjust** button (pending salaries, principal only) opening a Manual Adjustment dialog (Extra Allowance/Bonus, Extra Deduction, Note → `PATCH /salaries/{id}`, net recalculates + toast). Generate, Salary Structure, Pay, Slip download, and Email buttons already existed. data-testids: adjust-salary-<id>, adjust-allowance, adjust-deduction, adjust-note, save-adjust-btn.
- **VERIFIED (curl + screenshot)**: generate (gross ₹35,000 → net ₹32,712.83) → adjust (+₹2000 allowance, −₹500 deduction → net ₹34,212.83) → pay (slip SAL-…) → slip PDF 200. Salary page + Adjust dialog render correctly. NOTE: email-slip returns 502 for demo teachers because their `@edusync.in` emails are fake and the Resend proxy rejects them — real inboxes deliver.

## Certificate Redesign + Multi-Template Picker (2026-08-15, part 31) — DONE
- **Discarded old baked-text template**. Generated 3 clean, text-free landscape backgrounds (stored in `/app/backend/assets/`): `design_amber.png` (charcoal + gold rosette medal top-left, orange accents), `design_navy.png` (slim navy/gold ornamental border, large clean interior), `design_emerald.png` (emerald/gold corner waves + gold laurel). All have empty white/cream bodies — NO ghost/placeholder text.
- **True cursive script name font**: registered `GreatVibes-Regular.ttf` (name) and `EBGaramond-Regular.ttf` (serif body) in reportlab at startup (`CERT_FONT_SCRIPT` / `CERT_FONT_SERIF`; graceful Helvetica fallback).
- **Rewrote `certificate_pdf`** (clean overlays, no white masks): institute logo top-center; "CERTIFICATE" title (design-colored) + "OF <TYPE>" gold/accent subtitle; "This certificate is proudly presented to"; student **name in Great Vibes script** with accent underline; auto-filled description body (EB Garamond, wrapped ≤4 lines); **signatory name + designation bottom-left** (seal/logo stamped above the line); **date + institute location bottom-right**; QR + cert-no bottom-center. Sharp landscape A4 (841×595). Per-design title/accent colors.
- **Design picker**: `CertificateIn`/`BulkCertIn` gained `design` (amber/navy/emerald), stored on the cert doc and used at render. `Certificates.jsx` shows a 3-card visual picker (`cert-design-picker`, `cert-design-amber|navy|emerald`); passed on both single + bulk issue.
- **VERIFIED (rendered PNGs of all 3 designs + frontend screenshot)**: clean backgrounds, script name renders, no overlap, date/location/signature/QR aligned; picker selectable. Test certs cleaned up.

## Parent Notifications + Analytics Dashboard (2026-08-15, part 32) — DONE
- **Notification infra**: added `add_notification(institute_id, recipient_id, type, title)` (persists to `db.notifications` per student) and `notify_student(...)` which persists in-app + fires WhatsApp/SMS (Twilio, graceful) + email (Resend, graceful). Twilio IS configured on this pod but on the free trial 50/day cap — sends silently skip past the cap, in-app always works.
- **Events wired to notify the parent/student**: attendance (present AND absent, only on status change — `_mark_student`), fee generated (`create_fee` + monthly-fee cron `_generate_monthly_fees`), fee paid (`mark_fee_paid`, razorpay `rzp_verify`, `pay_partial`), certificate issued (`create_certificate` + `bulk_certificates`).
- **Parent/student notification feed**: `GET /notifications` now has a parent/student branch returning persisted notifications (last 20) + unread count + recent announcements. `POST /notifications/mark-read` marks all read. `Layout.jsx` NotificationBell marks read on open and has new icon types (attendance, certificate, fee_paid). Works for the parent portal bell.
- **Analytics** (`GET /dashboard/analytics`, principal): fee trend 6mo (billed vs collected + rate), overall collection rate, attendance heatmap (class × last 7 days, color-graded), class-wise avg exam performance, defaulters by class (count + dues). New page `frontend/src/pages/Analytics.jsx` at route `/app/analytics` + sidebar nav (`TrendingUp`) + "Analytics" CTA button on the principal dashboard (`open-analytics-btn`).
- **VERIFIED (curl + screenshots)**: analytics endpoint returns live data (58.9% rate, ₹92.5k collected); attendance/fee/certificate all persist a parent notification (curl); Analytics page renders all 4 widgets; parent bell shows notifications with correct icons + mark-read. NOTE: fixed a duplicate-tail corruption in Layout.jsx from a bad search_replace (removed `iv>` garbage, re-added markRead).
- data-testids: analytics-page, an-collection-rate/collected/outstanding/billed, panel-fee-trend/class-performance/heatmap/defaulters, heat-<class>-<i>, defaulter-row-<class>, open-analytics-btn.

## Analytics Date Range + Notification Center + Attendance Drilldown (2026-08-15, part 33) — DONE
- **Analytics date range**: `GET /dashboard/analytics` now accepts `months` (3/6/12) or `from_month`/`to_month` (YYYY-MM custom). Response adds `period` and `heatmap.full_days`. Fee trend + collection KPIs and class performance (exams filtered by exam_date month, undated always counted) respect the range. `_months_between()` helper added. `Analytics.jsx` has a pill selector: This Term (Apr–Sep or Oct–Mar auto), 3M/6M/12M, Custom (two month inputs + Apply). testids: analytics-range, range-term/3m/6m/12m/custom, range-from/to/apply.
- **Attendance drilldown**: `GET /attendance/absentees?batch_id=&date_str=` returns absent students (name, id, roll, parent_phone). Heatmap cells are now clickable buttons (`heat-<class>-<i>`) → opens `absentees-dialog` listing absentees for that class+day, each with a free wa.me WhatsApp link. Heatmap rows carry `batch_id`.
- **Notification Center**: `GET /notifications/history?kind=all|fees|attendance|certificates` returns up to 300 persisted notifications for the student/parent. New page `frontend/src/pages/NotificationCenter.jsx` at `/app/notifications` with filter tabs + relative timestamps, marks all read on mount. Added to parent & student sidebar nav; bell dropdown has a "View all notifications" footer (student/parent). testids: notification-center, notif-tabs, notif-tab-<kind>, notif-row-<i>, view-all-notifications.
- **VERIFIED (curl + screenshots)**: analytics 12m/custom ranges return correct period + trend; absentees returns the right student; notification center renders with working filter tabs; drilldown dialog opens from a heatmap cell. Bulk **"Message all N parents on WhatsApp"** button (`message-all-absent`) in the absentees dialog opens a wa.me chat per absent parent (free click-to-chat). NOTE: hit the recurring large-file duplicate-tail corruption twice (Layout.jsx tail + a stray server.py fragment at old line 4142) — both fixed and re-verified with ast.parse + babel.
## ID Card Orientation (Horizontal + Vertical) (2026-08-15, part 34) — DONE
- ID cards are DOM/React (`components/IDCard.jsx`) printed via `window.print()` (no backend PDF). Reworked to accept an `orientation` prop ("landscape"|"portrait"), both strictly CR80: landscape 85.6mm×54mm, portrait 54mm×85.6mm.
- Templates staged in `frontend/public/`: `id-template-h.png` (provided landscape master) and `id-template-v.png` (AI-generated matching navy/gold/green vertical theme). Used as full-card `backgroundSize:100% 100%`; fields (photo, institute name, heading, name, ID, detail rows, QR) absolutely positioned per orientation so layout auto-adjusts. Works for both student and faculty variants.
