...
- **VERIFIED (curl + screenshots)**: analytics 12m/custom ranges return correct period + trend; absentees returns the right student; notification center renders with working filter tabs; drilldown dialog opens from a heatmap cell. NOTE: hit the recurring large-file duplicate-tail corruption twice (Layout.jsx tail + a stray server.py fragment at old line 4142) — both fixed and re-verified with ast.parse + babel.
